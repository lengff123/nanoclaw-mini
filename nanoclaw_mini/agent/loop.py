...
from nanoclaw_mini.providers.base import LLMProvider
...
class AgentLoop:
    ...
    def _tool_hint(tool_calls: list) -> str:
        """Format tool calls as concise hint, e.g. 'read_file(\"path\")'."""
        def _fmt(tc):
    ...
    async def _handle_restart(self, msg: InboundMessage) -> None:
        ...
        async def _do_restart():
            ...
            # Use -m nanoclaw_mini instead of sys.argv[0] for Windows compatibility
            # (sys.argv[0] may be just "nanoclaw-mini" without full path on Windows)
            os.execv(sys.executable, [sys.executable, "-m", "nanoclaw_mini"] + sys.argv[1:])
