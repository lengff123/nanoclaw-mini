...
In `nanoclaw-mini`, prefer the active Codex login and an explicit OpenAI model, for example:

```bash
summarize "https://example.com" --model openai/gpt-5.2
```

Outside this repo, `summarize` can also use the provider environment variables documented by its own project.
